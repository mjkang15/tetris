...
- 진입점은 `index.html`을 루트에 두어야 합니다.
- GitHub Pages는 루트의 `index.html`을 기본 페이지로 인식합니다.
- `index.html` → `game.html` 링크가 상대 경로(`./game.html`)로 되어 있으면 별도 수정 없이 동작합니다.
- 저장소 루트가 아닌 하위 폴더(예: `frontend/`)를 배포하려면 **Branch + 폴더** 대신 **GitHub Actions**를 사용하거나, `frontend/` 내용을 루트로 옮기는 것이 간단합니다.

---

## 참고: 하위 폴더만 배포하기 (GitHub Actions 사용)

저장소 루트가 아닌 `frontend/` 폴더만 Pages로 서비스하고 싶을 때:
